	...
	while(defined($_=$lines[0]))
	{
		...
		elsif(/^(1\.|[\*\+\-]) /) # lists
		{
			...
			while($lines[0]=~/^($re)(?: |\t)(.*)/)
			{
				...
				if($skip) { while(@lines and $lines[0]=~/^\s*$/) { shift @lines; } } # skip empty lines ###comment this out for PROGRESS
			}
			$res.="<$tag>$html</$tag>";
		}
		elsif(/^(?:    |\t)/) # code sections
...
sub do_spans($@)
{
	...
	return join "<br />",map
	{
		...
		# do <strike>
		$line=~s{ (?<![0-9a-zA-Z\*_\x80-\x9f\xe0-\xfc]) (\~\~|___) (?![<>\s\*_]) ([^<>]+?) (?<![<>\s\*_\x80-\x9f\xe0-\xfc]) \1 (?![0-9a-zA-Z\*_]) }{<strike>$2</strike>}gx;
        
        # do <spoiler>
        $line=~s{ (?<![0-9a-zA-Z\*_\x80-\x9f\xe0-\xfc]) (\*\*|____) (?![<>\s\*_]) ([^<>]+?) (?<![<>\s\*_\x80-\x9f\xe0-\xfc]) \1 (?![0-9a-zA-Z\*_]) }{<p style="width: max-content; max-width: 90vw; word-wrap: break-word;"><span class="spoiler">$2</span></p>}gx;
        
		# do <details>
		$line=~s{ (?<![0-9a-zA-Z\*_\x80-\x9f\xe0-\xfc]) (\+\+|_____) (?![<>\s\*_]) ([^<>]+?) (?<![<>\s\*_\x80-\x9f\xe0-\xfc]) \1 (?![0-9a-zA-Z\*_]) }{<details><summary style="padding: 4x; width: 130px; background-color: #FFF2F2; border: none; box-shadow: 1px 1px 2px #bbbbbb; cursor: pointer;">Surprise box!!!</summary><p style="width: max-content; max-width: 90vw; background-color: #FFF2F2; padding: 4px; margin: 0; box-shadow: 1px 1px 2px #bbbbbb; word-wrap: break-word;">$2</p></details>}gx;
